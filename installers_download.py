...
installer_filename = []
installer_path = r"C:\sw\Jenkins\workspace\\"
deletedir = []
...
def download_installers(app_url):
    ...
    ''' giving exact path to download filename i.e; C:\sw\Jenkins\workspace\niminstall_uimserver_*.zip '''
    ...
    if exit_code == 0:
        ...
        if exit_code == 0:
            ...
            for deletedirfile in deletedir:
                delete_dir = "echo y | del /s  {}".format(deletedirfile) if ".zip" in deletedirfile else "echo y | rmdir /s  {}".format(deletedirfile)
